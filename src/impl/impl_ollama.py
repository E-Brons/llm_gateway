...
class OllamaGeneralLLM(GeneralLLM):
    ...
    def complete(self, messages: list[dict]) -> TextResponse:
        ...
        content, used_model = _ollama_chat(
            self.ollama_url, self.model, messages, self.timeout,
            temperature=self.temperature, max_tokens=self.max_tokens,
            response_schema=self.response_schema,
        )
        return TextResponse(content=content, model=used_model,
...
class OllamaTextGenLLM(TextGenLLM):
    ...
    def complete(self, messages: list[dict], *, max_retries: int = 3) -> TextResponse:
        def call_fn(msgs: list[dict]) -> tuple[str, str]:
            return _ollama_chat(self.ollama_url, self.model, msgs, self.timeout,
                                temperature=self.temperature, max_tokens=self.max_tokens,
                                response_schema=self.response_schema)
...
class OllamaImageGenLLM(ImageGenLLM):
    ...
    def generate(self, prompt: str, *, max_retries: int = 3,
        ...
        def call_fn() -> tuple[bytes, str]:
            payload: dict = {
                "model": _bare_model(self.model), "prompt": prompt, "stream": False,
            }
            if images_b64:
    ...
    def inspect(self, image: bytes, system: str, prompt: str, *,
        ...
        def call_fn(msgs: list[dict]) -> tuple[str, str]:
            return _ollama_generate(
                ...
                system=system, images=[image_b64],
                temperature=self.temperature, max_tokens=self.max_tokens,
                response_schema=self.response_schema,
            )
